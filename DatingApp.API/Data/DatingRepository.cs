...
namespace DatingApp.API.Data
{
    public class DatingRepository : IDatingRepository
    {
        ...
        private async Task<IEnumerable<int>> GetUsersLikes(int userId, bool likers) 
        {
        ...
        public async Task<PagedList<Message>> GetMessagesForUser(MessageParams messageParams)
        {
            var messages =  _context.Messages.Include(u => u.Sender).ThenInclude(p => p.Photos)
                                .Include(u => u.Rescipient).ThenInclude(p => p.Photos).AsQueryable();

            switch (messageParams.MessageContainer)
            {
                case "Inbox":
                    messages = messages.Where(u => u.RescipientId == messageParams.UserId);
                    break;
                case "Outbox":
                    messages = messages.Where(u => u.SenderId == messageParams.UserId);
                    break;
                default:
                messages = messages.Where(u => u.RescipientId == messageParams.UserId && u.IsRead == false);
                    break;
            }
        ...
        public async Task<IEnumerable<Message>> GetMessageThread(int userId, int recipientId)
        {
            var messages = await _context.Messages
                ...
                .Include(u => u.Rescipient).ThenInclude(p => p.Photos)
                .Where(m => m.RescipientId == userId && m.RecipientDeleted == false 
                    && m.SenderId == recipientId 
                    || m.RescipientId == recipientId && m.SenderId == userId 
